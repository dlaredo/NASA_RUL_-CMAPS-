...
class SequenceDataHandler():
	...
	def generate_train_data(self, unroll=True):
		...
		for i in range(num_samples):
			for j in range(sequences_per_sample[i]):
				...
				# print("self._y_train_list:",self._y_train_list[i])
	...
	def generate_test_data(self, unroll=True):
		...
		#Different shapes for the arrays depending wether data is to be unrolled or not
		if unroll == True:
			self._X_test, self._y_test = np.empty([num_samples, self._feature_size*self._sequence_length]), np.empty([num_samples, self._y_test_list[0].shape[1]])
		else:
			self._X_test, self._y_test = np.empty([num_samples, self._sequence_length, self._feature_size]), np.empty([num_samples, self._y_test_list[0].shape[1]])

		for i in range(num_samples):
			sequence_samples = self._X_test_list[i][-self._sequence_length:,:]
			print("sequence_samples test:", self._y_test_list[i].shape)
	...
	def generate_crossValidation_data(self, unroll=True):
		...
		#Different shapes for the arrays depending wether data is to be unrolled or not
		if unroll == True:
			self._X_crossVal, self._y_crossVal = np.empty([num_samples, self._feature_size*self._sequence_length]), np.empty([num_samples, self._y_crossVal_list[0].shape[1]])
		else:
			self._X_crossVal, self._y_crossVal = np.empty([num_samples, self._sequence_length, self._feature_size]), np.empty([num_samples, self._y_crossVal_list[0].shape[1]])

		for i in range(num_samples):
			sequence_samples = self._X_crossVal_list[i][-self._sequence_length:,:]
			print("sequence_samples cross validation:", self._y_crossVal_list[i].shape)
